...
cmd({
  ...
  category: "owner",
  filename: __filename
}, async (conn, mek, m, { from, isCreator, sender }) => {
  try {
    const isGroup = from.endsWith('@g.us');
    const ownerDM = "254112192119@s.whatsapp.net"; // Replace with owner's number

    if (!isCreator) {
      const responseText = `*📛 Unauthorized Access*\n\nUser: ${sender.split('@')[0]}\nTried to use owner command in: ${isGroup ? 'Group' : 'DM'}`;
      
      // Send notification to owner's DM
      await conn.sendMessage(ownerDM, {
        text: responseText,
        contextInfo: {
          forwardingScore: 999,
          isForwarded: true,
    ...
    // Send notification to owner if command was used in a group
    if (isGroup) {
      const notificationText = `*🔔 Command Used in Group*\n\nGroup: ${from}\nUser: ${sender.split('@')[0]}\nCommand: ${m.text}`;
      await conn.sendMessage(ownerDM, {
